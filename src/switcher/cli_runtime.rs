...
use crate::switcher::{
    CANCELLED_MESSAGE, ReloadAppTarget, UpdateAction, UpdateConfig, UpdatePromptOutcome,
    command_name, delete_profile, detect_codex_app, edit_config, effective_reload_target,
    ensure_paths, format_cancel, list_profiles, load_profile, migrate_profiles,
    package_command_name, print_output_block, relay_login, reload_app, resolve_paths,
    run_update_prompt_if_needed, save_profile, set_plain, show_config, status_label,
    status_profiles, switch_best_profile, switch_reload_target, sync_current_readonly,
    unreserve_profile, use_color_stdout,
};
...
fn run(cli: Cli) -> Result<(), String> {
    ...
    match cli.command {
        ...
        Commands::Load {
            label,
            reload_app: reload_app_target,
            no_reload_app,
        } => {
            let reload_target = if no_reload_app {
                None
            } else {
                switch_reload_target(&paths, reload_app_target)?
            };
        ...
        Commands::Reserve { label } => crate::switcher::reserve_profile(&paths, label),
