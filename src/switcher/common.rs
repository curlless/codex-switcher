...
pub fn command_name() -> &'static str {
    ...
    COMMAND_NAME
        .get_or_init(|| {
            let env_value = env::var("CODEX_SWITCHER_COMMAND")
                .ok()
...
#[cfg(any(not(test), all(test, not(feature = "switcher-unit-tests"))))]
...
pub fn resolve_paths() -> Result<Paths, String> {
    ...
    let auth_codex = env::var_os("CODEX_SWITCHER_AUTH_DIR")
        .or_else(|| env::var_os("CODEX_PROFILES_AUTH_DIR"))
        .map(PathBuf::from)
...
fn resolve_home_dir() -> Option<PathBuf> {
    let codex_home = env::var_os("CODEX_SWITCHER_HOME")
